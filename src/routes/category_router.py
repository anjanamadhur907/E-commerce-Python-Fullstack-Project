...
from src.dependency.service_dependency import get_category_service, authentication
from src.service.category_service import CategoryService

router = APIRouter(prefix="/category", tags=["Category"])

@router.post("/", status_code=201)
async def create_category(payload:dict=Depends(authentication),
...
async def delete_category(id:int,payload:dict=Depends(authentication), category_service:CategoryService=Depends(get_category_service)):
    return await category_service.delete_category(id)

@router.get("/", status_code=200)
async def fetch_all_category(payload:dict=Depends(authentication),category_service:CategoryService=Depends(get_category_service)):
