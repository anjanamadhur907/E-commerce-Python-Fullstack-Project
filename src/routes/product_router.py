...
from src.dependency.service_dependency import get_product_service, authentication
from src.service.product_service import ProductService

router = APIRouter(prefix="/product", tags=["product"])

@router.post("/", status_code=201)
async def create_product(payload:dict=Depends(authentication),
                         title:str=Form(...),
                         price:Decimal=Form(...),
...
async def delete_product(id:int,payload:dict=Depends(authentication), product_service:ProductService=Depends(get_product_service)):
